...
func TestProcessStubsSequential(t *testing.T) {
	searcher := newSearcher()

	// Create few test stubs to trigger sequential processing
	stubs := make([]*Stub, 50)
	for i := range 50 {
		stubs[i] = &Stub{
			ID:       uuid.New(),
	...
	query := QueryV2{
		Service: "test.service",
		Method:  "TestMethod",
		Input:   []map[string]any{{"id": "25"}},
	}

	result, err := searcher.processStubs(query, stubs)
	require.NoError(t, err)
	require.NotNil(t, result.Found())
	require.Equal(t, "stub25", result.Found().Output.Data["result"])
}
